...
let rec xor x y = if x then if y then false else true else y in
...
let rec solver_rect_surface m dirvec b0 b1 b2 i0 i1 i2  =
  ...
  if (fabs (d2 *. dirvec.(i1) +. b1)) < abc.(i1) then
    if (fabs (d2 *. dirvec.(i2) +. b2)) < abc.(i2) then
...
let rec solver_rect_fast m v dconst b0 b1 b2 =
  ...
  if  (* YZ平面との衝突判定 *)
    if (fabs (d0 *. v.(1) +. b1)) < (o_param_b m) then
      if (fabs (d0 *. v.(2) +. b2)) < (o_param_c m) then
  ...
  if  (* ZX平面との衝突判定 *)
    if (fabs (d1 *. v.(0) +. b0)) < (o_param_a m) then
      if (fabs (d1 *. v.(2) +. b2)) < (o_param_c m) then
  ...
  if  (* XY平面との衝突判定 *)
    if (fabs (d2 *. v.(0) +. b0)) < (o_param_a m) then
      if (fabs (d2 *. v.(1) +. b1)) < (o_param_b m) then
...
let rec is_rect_outside m p0 p1 p2 =
  if 
    if (fabs p0) < (o_param_a m) then
      if (fabs p1) < (o_param_b m) then
	(fabs p2) < (o_param_c m)
      else false
    else false
...
let rec shadow_check_and_group iand_ofs and_group =
  if and_group.(iand_ofs) = -1 then
    ...
    if (if t0 <> 0 then t0p < (-0.2) else false) then 
