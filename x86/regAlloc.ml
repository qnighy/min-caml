...
let rec g dest cont regenv el = (* 命令列のレジスタ割り当て (caml2html: regalloc_g) *)
  match el.loc_val with
...
and g' dest cont regenv el = (* 各命令のレジスタ割り当て (caml2html: regalloc_gprime) *)
...
and g'_if dest cont regenv exp constr e1 e2 = (* ifのレジスタ割り当て (caml2html: regalloc_if) *)
  ...
  ((List.fold_left
     (fun e x ->
       ...
       loc_inherit e (seq(Save(M.find x regenv, x), e))) (* そうでない変数は分岐直前にセーブ *)
