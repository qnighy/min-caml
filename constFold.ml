...
let rec g env el = (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  ...
  | IfEq(x, y, e1, e2) when memi x env && memi y env -> (if findi x env = findi y env then g env e1 else g env e2).loc_val
  | IfEq(x, y, e1, e2) when memf x env && memf y env -> (if findf x env = findf y env then g env e1 else g env e2).loc_val
  | IfEq(x, y, e1, e2) -> IfEq(x, y, g env e1, g env e2)
  | IfLE(x, y, e1, e2) when memi x env && memi y env -> (if findi x env <= findi y env then g env e1 else g env e2).loc_val
  | IfLE(x, y, e1, e2) when memf x env && memf y env -> (if findf x env <= findf y env then g env e1 else g env e2).loc_val
  | IfLE(x, y, e1, e2) -> IfLE(x, y, g env e1, g env e2)
  | Let((x, t), e1, e2) -> (* letのケース (caml2html: constfold_let) *)
      let e1' = g env e1 in
      let e2' = g (M.add x e1'.loc_val env) e2 in
      Let((x, t), e1', e2')
  | LetRec({ name = x; args = ys; body = e1 }, e2) ->
      LetRec({ name = x; args = ys; body = g env e1 }, g env e2)
  | LetTuple(xts, y, e) when memt y env ->
      List.fold_left2
	(fun e' xt z -> Let(xt, loc_inherit el (Var(z)), loc_inherit e e'))
	(g env e).loc_val
	xts
	(findt y env)
